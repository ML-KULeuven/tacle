...
In this work we focus on the spreadsheet data, which is physically one big table, but conceptually consists of multiple tables, see Fig \ref{fig:main_example} for an example. Formally, a table is $n \times m$ matrix, each entry table is called  a \textit{cell}.

Each cell has a {\em type} which can be numeric or textual. The numeric type has two subtypes: integers and floats. 
We also consider the special element called \textit{None}, which has two types: numeric and textual.
...
\end{itemize}
For notational convenience, we will refer to cells with $C$,  to vectors with $V$, and to groups and subgroups with $G$. 
...
A \textit{constraint} definition is a triple \textit{(\CName, \CSignature, \CFunction)}. Let us elaborate on each of them. 
\begin{itemize}
\item
\textit{\CName} is the name of the constraint together with the name of the groups, 
\item \textit{\CSignature} is specifying a set of restrictions on the types and variables of the constraint, 
% the properties of the group assignments, corresponding to the variables in \CName, such as their types, 
e.g., requiring the variables to be integers or constraining the sizes, e.g. the length of vectors in the arguments must be equal. These are restrictions on the group meta-information not on the actual group content. 
\item \textit{\CFunction} defines the actual constraint that needs to be satisfied in the data.
\end{itemize}
