...
\begin{itemize}
  ...
  \item
  A \textbf{subgroup} $G'$ of a group $G$ is simply a projection of $G$ onto a consecutive subset of its vectors. % is a subrange of vectors belonging to a group $G$. If the subgroup $g$ must contain only one vector, we write $g \in G$. %It defines what it means for a constraint to be well-formed
