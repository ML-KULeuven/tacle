...
\item
  ...
  The constraints act on properties or group meta-information, not on the actual group content. In ILP, it is known as mode declarations, when one needs to declare types of variables in the vocabulary. Also, it is analogous to standard linear algebra well-defined requirements, e.g., in order to multiply matrices $A$ and $B$, $A$ must have the size of $n \times k$ and $B$ the size $k \times m$.
\item \textit{\CFunction} is a constraint specifying that the data in the subgroups satisfies the function. In ILP, it is known as bias, where we constrain ourselves to only particular subset of language in which we perform the search. 
\end{itemize}
...
%\begin{itemize}
%\item \CName = \textit{$v_r$ = RANK($v_x$)}, where $v_r$ and $v_x$ are %the variables;
...
Therefore, if there is a solid edge from constraint~$C_1$ to constraint~$C_2$, then a set of subgroups satisfying~$C_2$ must contain the subgroups satisfying~$C_1$.
...
\begin{table*}
  ...
  \begin{tabularx}{\textwidth}{l X X}
    ...
    \ecalldiff{v_x} & $\discrete(v_x)$ & All values in $v_x$ are different: $v_x[i] \neq v_x[j]$ if $i \neq j$ \\
    %X = Y & & \\
    \ecperm{v_x}
  ...
  \end{tabularx}
  \caption{Spreadsheet Constraints \sergey{Here a detailed explanation on what is going on, what is essential, what is not? etc should be self-explanatory}\samuel{Motivate why product, not sum (=diff) you only need one, hardcoded preference for display}
}
