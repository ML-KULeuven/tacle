...
Given a constraint template~$s$ and the set of all blocks~\blocks, the goal is to find all combinations of blocks that satisfy the constraint signature. Let there be $n$ blocks and let~$s$ have~$k$ arguments, then a simple generate-and-test method would have to test $n^k$ combinations (given that blocks can be repeated), which can grow very rapidly with $n$, depending on $k$.
