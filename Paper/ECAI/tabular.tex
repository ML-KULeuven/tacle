...
\begin{figure*}[thb]
  ...
  \end{center}
  \vspace{-10pt}
  \caption{Example spreadsheet (black words and numbers only). Green background indicate headerless tables, dark borders indicate maximal type-consistent blocks.}
...
%
  ...
  $^1$(Spurious constraints) $^2$(Redundant constraints not originally present)}
...
\begin{example}
Consider the (headerless) table~$T_1$ in Figure~\ref{fig:main_example}, its rows are not type consistent (i.e. they contain both numeric and textual data).
...
\item the \textit{\CSignature} defines the requirements that the arguments of the predicate must satisfy.
...
In terms of logical and relational learning \cite{luc_book}, the \CSignature is known as the {\em bias} of the learner, it specifies when the constraint its arguments are well-formed.
...
Furthermore, few constraint solvers support floating point numbers, which are prevalent in spreadsheets.
...
Moreover, \sname currently uses product (\ecprod{B_R'}{B_1'}{B_2'}) instead of division (\eccalc{B_1'}{B_R' / B_2'}) and difference (\ecdiff{B_r}{B_1}{B_2}) instead of sum (\eccalc{B_1}{B_r + B_2}).
These choices are hardcoded preferences based on the prevalence of difference and product in financial spreadsheet data, however, these constraints can be trivially rewritten if necessary.
...
We studied various questions, most notably with what accuracy our algorithm can find constraints.
...
There are three main categories of spreadsheets in the benchmark: spreadsheets from an exercise session teaching Excel, spreadsheets from tutorials online and publicly available spreadsheets such as crime statistics or financial reports that demonstrate more real world usage\footnote{(attached to to the submitted paper)}.
...
This example combines several smaller exercise session examples into one spreadsheet.

\subsubsection{Results}
\sname takes a few seconds to find the 18 constraints in Figure~\ref{fig:sol_example}.
...
Moreover, there is 1 \textit{LOOKUP} constraint that was is present in the original spreadsheet (looking up ID based on Salesperson).

For this example our primary goal of finding all original constraints is achieved.
The implementation also returns 6 additional constraints ($33.33\%$ of the total).
