...
Hence understanding and learning constraints in tabular data is a novel and challenging problem for machine learning.
...
on learning both column- and row-constraints on integer, floating point as well as textual data.
...
\begin{figure*}[thb]
  ...
  \caption{
    ...
    Most tables only contain type-consistent columns, however, table~$T_2$ also contains type-consistent rows.
    Due to the location of the header of~$T_2$ we can infer that it consists of row data.
...
%
      &~\ecrank{\range{T_{1}}{\rangeall}{1}}{\range{T_{1}}{\rangeall}{5}}^* , \ecrank{\range{T_{1}}{\rangeall}{1}}{\range{T_{1}}{\rangeall}{6}}^* \\
      &~\ecrank{\range{T_{1}}{\rangeall}{1}}{\range{T_{1}}{\rangeall}{10}}^*, \ecrank{\range{T_{1}}{\rangeall}{8}}{\range{T_{1}}{\rangeall}{7}} \\
...
\caption{
  ...
  Templates marked with $\dagger$ are \textit{aggregate} templates, the table shows $SUM$ but \sname also supports $MAX$, $MIN$, $AVERAGE$, $PRODUCT$ and $COUNT$.
...
\end{itemize}
$SUM$ is an aggregate constraint, and a similar constraint is available for aggregate operators $MIN$, $MAX$, $AVERAGE$, $PRODUCT$ and $COUNT$. The \textit{conditional} aggregate variant $SUMIF$ only ranges over those cells that satisfy a condition on a related cell in that table, e.g. $\range{T_1}{\rangeall}{10}$ and the data in $T_5$ for matching salesperson names.
\end{example}
...
Likewise, the template is satisfied if both the signature and definition are satisfied; in logic programming, we would define the predicate~$s$ using a Prolog like clause: $s(B_1, ..., B_n) \leftarrow \sigc(B_1, ..., B_n) \wedge \defc(B_1, ..., B_n)$.
...
Two different constraints can also be logically equivalent, due to their nature e.g. for addition/subtraction and product/division: $\ecprod{\sg_r}{\sg_1}{\sg_2} \equiv \ecdiv{\sg_1}{\sg_r}{\sg_2}$.
...
In Section~\ref{sec:opts} we describe a number of optimizations designed to speed up the algorithm.
...
%Br and Bx are numeric; rows(Bx) ⇤ 2; and columns(Bx ) = length(Br )
  ...
  Consider the constraint template $\ecsumr{\sg_r}{\mathbf{\sg_x}}$, then the generated CSP will contain two variables $V_r, V_x$ corresponding to arguments~$\sg_r$ and~$\mathbf{\sg_x}$.
  Given maximal blocks~\blocks, the domain of these variables are $D(V_r) = D(V_x) = \{ 1, \dots, |\blocks| \}$.
  Finally, the signature of $\ecsumr{\sg_r}{\mathbf{\sg_x}}$ (see Table~\ref{table:constraints}) will be translated into constraints:
  \begin{align*}
...
\begin{example}
  ...
  Note that this assignment does not satisfy the signature yet because $\sg_r$ contains more than one vector.
  This step aims to generate subassignments $(B_r' \sqsubseteq B_r, \mathbf{B_x}' \sqsubseteq \mathbf{B_x})$ that do satisfy the signature and test whether they satisfy the definition:
...
On the other hand, a simple generate-and-test will usually suffice.
...
This is made possible by adding, on line \ref{algo:tcl:super}, the set of learned constraints $C$ to the argument of \generategroups.

This is used to speed up the \generategroups step. For constraint templates where some arguments are also part of a constraint that it depends on, it is not needed to search for superblocks and subblocks from scratch. Instead, one can start from all and only the actual argument assignments of the depending constraint, and only search matching superblocks for the remaining arguments.

\begin{example}
  Consider $\ecfkey{B_{fk}}{B_{pk}}$, which states that every value in $B_{fk}$ also exists in $B_{pk}$; its signature includes $\ecalldiff{B_{pk}}$. There are 16 \textit{ALLDIFFERENT} constraints to be found in Figure~\ref{fig:main_example}, hence, the \generategroups only needs to check which superblocks for $B_{pk}$ are compatible (different table, same type) with these 16 assignments to $B_{fk}$.
\end{example}
...
The procedure to generate CSPs remains the same otherwise. %, except that the known subassignment(s) are added as extra blocks that can only be assigned to their corresponding variable.

%This approach has the additional benefit of ensuring that the (sub)blocks assigned to the dependent arguments already satisfy both the signature and definition of the template it depends on.
...
Moreover, among the semantically equivalent constraints product (\ecprod{B_R'}{B_1'}{B_2'}) and division (\eccalc{B_1'}{B_R' / B_2'}) we only added product, as well as only difference (\ecdiff{B_r}{B_1}{B_2}) and not sum (\eccalc{B_1}{B_r + B_2}). However, division and sum could easily be added, as could it be added explicitly in post-processing based on the matching product and difference constraints.
...
Should the target constraint be $\eccalc{B_2}{\mathit{MAX}(B_1[1{:}2])}$, then the last vector of~$B_1$ must be split off into a separate block before the above approach could find it.
...
1)~spreadsheets from an exercise session for teaching Excel; 2)~spreadsheets from tutorials online; and 3)~publicly available \textit{data} spreadsheets such as real-world crime statistics or financial reports\footnote{(attached to to the submitted paper)}.
...
We encoded all relevant constraints using our representation (in canonical form).
...
Constraints that are currently outside of the scope of this system are, for example, generic nested mathematical or logical formulas.


We focus on \textit{functional} constraints that could be used in spreadsheets, ignoring the structural constraints \textit{ALLDIFFERENT}, \textit{FOREIGNKEY} and \textit{PERMUTATION}.
We included detected equalities as relevant constraints (in 5~spreadsheets).
...
These include 5 spurious $\mathit{RANK}$ constraints, e.g. $\ecrank{\range{T_1}{\rangeall}{1}}{\range{T_1}{\rangeall}{5}}$, that are true by accident.
...
For this example our primary goal of finding all original constraints is achieved, the recall (i.e. $\frac{\text{relevant discovered}}{\text{relevant}}$) is~$1.0$.
The implementation also returns 6 additional constraints, therefore, our precision (i.e. $\frac{\text{relevant discovered}}{\text{all discovered}}$) is $12/18 = 0.67$, which is rather low compared to, for example, the spreadsheets in the benchmark.
...
Notice that for the data category the average number of tables and cells is higher while the number of relevant constraints is lower compared to the other categories.
...
\subsubsection*{Q1. How many relevant constraints are found by \sname?}
...
However, the overall precision per category reveals that the precision is much higher for the exercises and data categories ($0.97$ and $1.00$) and much lower for the tutorials category ($0.70$).
Moreover, the average precision per spreadsheet is, for all categories, over $0.90$.
...
Most of the execution time in these cases goes towards searching either aggregate constraints or conditional aggregates. \samuel{More details}
...
On many spreadsheets \sname achieves good results.
...
For example, we tested a spreadsheet (``exps\_tias'') with 6~tables, 531~cells and many duplicate columns across tables.
...
The running time can be improved by further optimizing the implementation, especially for conditional aggregates.
...
\textit{Functional} constraints, such as $\ecsumc{B_r}{\mathbf{B_x}}$, are able to compute a result~($B_r$) based on its input~($\mathbf{B_x}$).
...
Learning on a snapshot that does not contain line 17, we find constraints $\ecsumif{\range{T_2}{\rangeall}{2}}{\range{T_1}{\rangeall}{2}}{\range{T_2}{\rangeall}{1}}{\range{T_1}{\rangeall}{3}}$ and $\ecsumif{\range{T_2}{\rangeall}{3}}{\range{T_1}{\rangeall}{2}}{\range{T_2}{\rangeall}{1}}{\range{T_1}{\rangeall}{4}}$.
The new data $\Delta D$ consists of the new value \textit{West} on line 17.
...
\begin{center}
    \includegraphics[width=0.60\linewidth]{figures/autocompletion_example.png}
  \end{center}
...
Here, we look at \textit{conflict} constraints, i.e. constraints that were learned on the snapshot data but do not agree with the newly added data~$\Delta D$.
...
Therefore, conflicts should perhaps only be detected if the snapshot data is large enough.
...
If we define the sample to be the snapshot data and the other row(s) to be the additional input~$\Delta D$, this task can be formulated as online error detection and dealt with in the same way.
...
The resulting method has high accuracy, produces limited number of redundant constraints and is sufficiently efficient for normal as well as interactive use.
...
One other line of work, which can also mitigate redundant and spurious constraints, is to integrate the approach an interactive setting, where users can receive and provide feedback; more similar to an active learning setting. Other ways to handle redundant constraints include the (heuristic) filtering or post-processing of the constraint set.
