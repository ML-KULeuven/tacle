...
Of course, it is stated semi-informally here, but it already allows us to provide an intuition on the task. We (know) that often people work in spreadsheets with ranges, e.g. if we look at the Table 1  and at the first row in the Table 2 in Figure \ref{fig:main_example}, it is clear that one could have selected the first columns, clicked on the \textit{SUM} function and then just used the drag fill handle to apply the formula to the other columns. Similarly, that would work for the second (\textit{Average}), third (\textit{Min})  and forth (\textit{Max}) rows in Table 2. I.e., all operations are vectorized and make use of the standard spreadsheet functions. 
