...
In the previous section we introduced the problem of tabular constraint learning informally using the example in Figure \ref{fig:main_example}. Here we formalize the statement in terms of Excel constraints and group assignmentsd as follows: 
...
\begin{minipage}[c]{14em}
  ...
  \begin{tabular}{ll}
    ...
    \textbf{Find:}& for each constraint  $c$ in \constraints all the subgroups of $\groups$\\ 
    & satisfying the \CSignature and \CFunction of $c$ \\
  \end{tabular}
  \vspace{6pt}
\end{minipage}
...
\Return $S$
\end{algorithmic}
\caption{Tabular constraint learning \sergey{Samuel, I think \generategroups takes \dependencies as an argument}}
\label{algo:tcl}
\end{algorithm}
In theory each constraint is independent and equally useful. In practice, however, things are different. Assume that for some $X$ and $Y$ it holds that \textit{SUM(X)=Y}, and also we discovered that the vector $Y$ is equal to a vector $Z$. Then, of course, \textit{SUM(X)=Z} holds and it is a valid solution to the sum constraint problem. From a user's perspective the last solution is useless, since he or she already knows that $Y=Z$ and the latter constraint satisfaction follows. To capture this condensed representation \cite{condensed} of solutions, we have introduced a subsumption (generality) order between constraints, which is depicted using \textcolor{red}{dashed} lines in Figure \ref{fig:learning_order}. Besides generality, we know that certain constraints have as a subproblem other constraints, e.g. if $X$ is a \textit{series}, then it is also a \textit{permutation}. If discovered that $X$ is a not permutation, then $X$ should not be tested for the series constraint at all. To capture this solving strategy we introduced a constraint construction order depicted using \textcolor{green}{solid} lines in Figure \ref{fig:learning_order}.
...
\paragraph{Constraint ordering} $\constrainttorder(\constraints,\dependencies)$ uses the DAG \dependencies in Figure \ref{fig:learning_order} in the following way: all constraints are generated in the topological order of \dependencies. Also, if there is a construction edge from $C_1$ to $C_2$, and $X$ is not a solution to $C_1$, then $X$ is not generated as a candidate group for $C_2$. If there is a sumsumption edge from $C_1$ to $C_2$ and $X$ is a solution to $C_1$, then $X$ is not generated as a candidate group for $C_2$.
...
\begin{figure}[htb]
  ...
  \caption{Constraint Learning Order. The \textcolor{green}{solid} arrows indicate a construction order, i.e., learning one constraint is necessary to build the other. The \textcolor{red}{dashed} arrows indicate a subsumption order, the former constraint is used to prune the latter.}
