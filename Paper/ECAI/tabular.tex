...
\tias{todo me: check that more here}
...
All spreadsheets used in the experiments are in CSV format, and for each we have ran the table extraction tool once (using a combination of the rather primitive automatic detection + visual verification that the tables are meaningful (e.g. merge split-up tables because of None values, or remove headers of all-textual tables). \\ % split of weird sentence
...
These were determined by us using the formulas of the original sheet (if present), the headers in the sheet as well as general context if need. \tias{last part too vague!! Give example of where 'context' was needed/used. If nowhere, cut that part of sentence}.
...
Section~\ref{sec:which_cons} discusses the 32 constraints currently supported by \sname \tias{It does, right? Should also contain '32' number}. Some of the intended constraints are currently outside of the scope of \sname however, more specifically nested mathematical or nested logical formulas. We denote by \textbf{supported} constraints the subset of intended constraints that the system can find in theory.
...
In this section the impact on the running time is analyzed of 1)~the number of vectors; 2)~the length of the vectors; and 3)~the size of the blocks. \tias{Why only these and not the other? Motivated/explain, or also measure their effect ; )}

Recall that our method uses a 2-phase approach where in the first phase, the superblock assignments are generated (Section ...) and in the second phase, the subblock assignments are generated (Section ...).
...
\paragraph{Theoretical analysis of runtime, this does not belong in experiments}\tias{move to technical section}
...
As can be seen in Figure~\ref{fig:runtime_analysis}b\tias{This is the first figure discussed so must be a}, increasing the length of the vectors has a negligible impact on the performance for most template. Most of the increase in running time can be attributed to the RANK constraint which is less suited for failing fast \tias{Too vague, 'less suited', be precise}; the runtime for detecting all constraints except RANK is shown in green to highlight this.
...
Many constraints take single vector arguments, therefore, the number of subassignments \tias{unclear, this is about phase 2, the number to test, or the actual testing of each? explain and relate to Sec 4.3.2 what aspect you are discussing here} to be tested depends on the number of vectors in the spreadsheets and the arity of the constraint.
...
In practice, in our benchmark experiments, the assignment step of our algorithm is able to limit this rapid increase in running time for larger numbers of vectors \tias{How? explain, if you explain in the next sentence then start it with 'This is because' or alike, reads better}.
...
Therefore, the size of the blocks \tias{in what phase? the max. contigiuous? be more precise} in the spreadsheet also influences the runtime of these constraints.
...
The \tias{explain what THE experiment is, and how you set it up} synthetic experiments confirm that splitting a constant number of vectors into more, smaller blocks improves the performance of the aggregate constraint search (Figure~\ref{fig:runtime_analysis}c, green line).
...
Since the implementation of that last phase is more adept at generating these combinations and does not need to check for constraints on all these combinations, shifting this workload to the assignment \tias{more precise: to superblock?} phase slows down the algorithm. \tias{Point to figure to support this (actually, move that line into 4c??); also, if this is true, you are basically saying that the CP-based first phase is worse then the second phase? That kind-of demotivates the idea of using CP in first phase... What do you really want to share with the reader?}
...
While most spreadsheets can be analyzed very quickly, running times can go up to around $1s$ for others. ... \tias{Explain by relating to previous section of synthetic data}
...
Looking at the runtime per constraint template individually (including equality\tias{what does this mean?}), the slowest by far is fuzzy lookup using $\pm 36\%$ of the running time.
...
The intended constraints not discovered correspond to constraints not supported by the system (supported recall is 1). \tias{Rewrite and go into more detail about all supported found, some intended not support, which ones, what category, just some more details}
...
Examining the tutorial spreadsheets confirms that there are a few spreadsheets that have a high number of additional constraints. This is caused by duplicate data or multiple ways to calculate the same vectors. \tias{More details or examples; we copy intended constraints for equivalent/duplicate vectors, so not an issue?}
