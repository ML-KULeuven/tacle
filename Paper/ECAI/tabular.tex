...
\begin{figure}[thpb]
  \begin{center}
    \includegraphics[width=0.55\textwidth]{figures/Demo.png}
  \end{center}
  \vspace{-10pt}
...
\newcommand{\generategroups}{\textit{generateGroups}\xspace}
\newcommand{\extractgroups}{\textit{extractGroups}\xspace}
\newcommand{\findassignment}{\textit{findAssignment}\xspace}

\begin{algorithm}[thb]
  \begin{algorithmic}
    \footnotesize
    \State \textbf{Input:} $D$ -- dataset, (optional groups $G$)
    \State \textbf{Output:} $S$ -- learned constraints with their satisfaction assignment
    \If{$G$ is not defined}
       \State $G \gets \extractgroups(D)$
    \EndIf
  \For{$c \in \constraints$} \Comment{\constraints -- the set of predefined Excel constraints}
    \For{$G_1, \dots, G_n \in \generategroups(c,G)$}
      \State $S \gets S \cup \findassignment(c, G1, \dots, G_n)$
    \EndFor
  \EndFor\\
\Return $S$
\end{algorithmic}
