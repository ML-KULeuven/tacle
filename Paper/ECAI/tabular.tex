...
In the context of spreadsheets the variables typically refer to cells in tables, and solving the corresponding CSP problem means completing the tables so that all constraints in the spreadsheet are satisfied.

While the traditional CSP problem is given a set of variables and a set of constraints, and the task is to find an assignment, the {\bf inverse CSP} or the {\bf Tabular Constraint Learning Problem} can be defined as follows.\\
\begin{definition}[Tabular Constraint Learning]\label{def:problem_statement}
  Let ${\cal T}$ a set of instantiated tables (all cells have values),  ${\cal C}$ be a set of {\template}s, ${\cal G}$ be a set of groups  for the tables ${\cal T}$, then find the set of all constraints $s(G_1', ..., G_n')$ that are satisfied  in ${\cal T}$, for which all $G_i' \subseteq G_i \in {\cal G}$, and the groups $G_1', ... , G_n'$ satisfy the signature of the constraint template~$s \in {\cal C}$.
\end{definition}
% {\bf Given }
% \begin{enumerate}
% \item
...
that uses the tables of Figure 1, the signatures of Table 1 and the set of groups mentioned in (\samuel{show groups somewhere}).
...
\begin{definition}[Condensed Tabular Constraint Learning]\label{def:condensed_problem_statement}
  Let ${\cal T}$ a set of instantiated tables (all cells have values),  ${\cal C}$ be a set of {\template}s,  ${\cal G}$ be a set of groups for the tables ${\cal T}$, \dependencies be the set of all specifity rules, then find the set of all constraints $s(G_1', ..., G_n')$ that are satisfied  in ${\cal T}$, for which all $G_i' \subseteq G_i \in {\cal G}$, the groups $G_1', ... , G_n'$ satisfy the signature of the constraint template~$s \in {\cal C}$ and there is no $s'$ in ${\cal C}$ such that $s'(G_h',\dots,G_k')$ holds (for $1 \leq h \leq k \leq n$) and  $s'$ is more specific than $s$ in \dependencies.
\end{definition}
...
\sergey{should we show all the formulas here? or some maybe and say that the specificity DAG can be extracted?}
...
\samuel{application autocomplete}
