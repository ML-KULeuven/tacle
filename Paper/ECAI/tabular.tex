...
\item \CSignature: the two arguments $G_r$ and $G_x$ must be numeric groups and of size one; furthermore, the group $G_r$ must be integer (the rank) and the length of groups $G_x$ and $G_r$ must be identical, i.e., $length(G_r) = length(G_x)$;
...
From a FOL perspective, the name of the constraint (\textit{RANK}) is just the predicate, and the arguments $G_r$ and $G_x$ are the terms, which can be seen as either uninstantiated variables or as values (concrete groups with values).
...
Then we can also talk about constraints being satisfied. Basically, a constraint $c(G_1, ..., G_n)$ is {\em satisfied}  in a set of tables ${\cal T}$ if and only
...
\sergey{Samuel, probably it is better if you look here}
...
\begin{table*}
\medskip
Constraint templates marked with $*$ are \textit{structural}, they are discovered by our algorithm, however, they cannot be readily used in spreadsheet software.
...
Only bold subgroups may contain more than one vector.
\medskip
