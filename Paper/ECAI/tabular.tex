...
Based on the assumptions that most subassignments will not satisfy the definition of the constraint template, the implementation of the definition check is geared to \textit{fail-fast} when possible. Specifically, the entries of every subblock are fetched one by one, and as soon as one does not hold failure is returned. In many cases, the first entry will already not satisfy the definition (e.g. if $\sbl{r}[0] \neq \sbl{1}[0] \times \sbl{2}[0]$ then $\sbl{r} = \sbl{1} \times \sbl{2}$ does not hold).
As a result, many subassignments can be discared by looking at just one or a few entries, and considering all subassignments that will be checked, the runtime of the checks will typically not be influenced much by the length of the vectors.
\tias{Moved the discussion from 4.4.3 to here, where it better fits. Plus, I find the O(1) claim too strong! O() is worst-case time complexity, not average case}
...
We also added four structural constraints (\textit{ALLDIFFERENT}, \textit{PERMUTATION}, \textit{ORDER} and \textit{FOREIGNKEY}) so that they can be used in the signature of other constraint templates; they are also popular in constraint satisfaction~\cite{modelseeker}.


\tias{Just below this comment I commented out the paragraph that FK,Lookup,proj have optimized methods becuase it does not properly explain what is used instead. That creates confusion and makes our method not reproducible. Leave out until it is rewritten with sufficient information.}
...
%Instead of the more generic generate-and-test implementation, we use, for example, caching, maximal range-finding and partial tests. \tias{Really, this is unacceptably vague. I have no clue what you mean with that, meaning that our research is not reproducable and hence not scientific}
...
All spreadsheets used in the experiments are in CSV format, and for each we have ran the table extraction tool once using the visual tool. Manual intervention of the simple automatic detection was needed only in the case of None values (to merge split-up tables cause by the None's) and to remove all-textual headers of all-textual tables.%(using a combination of the rather primitive automatic detection + visual verification that the tables are meaningful (e.g. merge split-up tables because of None values, or remove headers of all-textual tables).
%\samuel{Should we not say just with the visual tool?} \tias{Rewrote a bit, I like the amount of detail as it provides insight into the limitations}
Blocks are then detected automatically using the block detection algorithm (Section~\ref{sec:make_groups}). Manual intervention was required for tables that contain \textit{None} values, as well as for (partially) numeric headers. \tias{This does not make sense to me: headers are removed before block detection; should this be part of manual intervention of table extraction tool??}
...
%\tias{I'm cutting this paragraph, in an unsupervised method you do not control the number of blocks and will hence not 'split unrelated vectors'; that would be supervised.}
...
\textbf{Runtime (s)} & 0.93 & 0.10 & 0.93 & 0.04 & 0.79 & 0.20
\end{tabular}
\end{table}}
...
While most spreadsheets can be processed very quickly, running times can go up to around $0.65s$ for others.
In the previous section we analyzed the effect of various factors that can help explain runtime behavior. \tias{Add something like 'on data generated to trigger many checks (equal sizes and lengths of many vectors)?? You choose}
...
The intended constraints that are not discovered correspond with nested constraints (e.g. $R = \frac{W}{(L/100)^2}$), variants of supported constraints (e.g. series that do not start at 1) or more complex versions of supported constraints (e.g. conditional aggregates that automatically infer the condition). \tias{if you can clarify the latter, please do; if too difficult OK but 'automatically infer' sounds.. unusual for Excel}
...
Since every aggregate can be calculated based on any of the (equal) columns, many of the resulting constraints will be marked as unintended. \tias{rewrite 'many of the..' into 'our method will find all of these; only one in 'true' set, others drive down precision...'}

Such unintended constraints are the result of multiple ways to calculate the same result, e.g. if there is duplicate data.
...
Instead of snapshots, one can repeatedly search for conflict constraints between the original spreadsheet and a sheet with a row or column removed. Many schemes can be devised for choosing which row(s) or column(s) are removed. \tias{Expand 'one can repeatedly search' into the actual steps: take spreadsheet, remove row/column, detect constraints and then search for any constraints not present in the full data (can easily be optimized to avoid known constraints btw)}
...
\tias{Smth about extendible: more constraints can be added by specifying signature in terms of properties and implementing the subblock finding (or in less detail, can be added, e.g. all native Excel constraints... should this be true)}
