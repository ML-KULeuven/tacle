...
Given a superblock assignment, the implementation for \textit{LOOKUP} first generates candidate vectors for \sbl{pk} and \sbl{val}, then generates a datastructure that allows it to quickly look up keys, afterwards it generates candidate vectors for the remaining arguments and, finally, checks which subassignments are valid.
...
For example, for conditional aggregates, superblock assignments are analyzed to find overlapping vectors.
