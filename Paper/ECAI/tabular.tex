...
Depending on the application, a solution to the tabular constraint learning problem can be considered to contain \textit{redundant} constraints. This is because constraints may be logically equivalent and constraints may also be implied by other constraints.

A constraint $c_1(G'_1, ... G'_n)$ \textit{implies} another constraint~$c_2(G''_1, ..., G''_m)$ where the second one ranges over a subsets of the arguments of the first, iff whenever the first constraint is true, the second must also be true. More formally, $\forall G'_1, ..., G'_n$ s.t. $\{G''_1, ..., G''_m\} \subseteq \{G'_1, ..., G'_n\},\, c_1(G'_1, ... G'_n) \rightarrow c_2(G''_1, ..., G''_m)$. We say that the first constraint is more specific than the second. \tias{Some reference?}

\paragraph{Example}
A first example is $permutation(G') \rightarrow alldifferent(G')$ (it can be seen in Table~\ref{table:constraints} that both constraints only hold if $G'$ is a vector). In this case the arguments are the same in both constraints. For most applications, it will be sufficient to output only the most specific one.
A more involved example is that a look-up constraint implies a foreign-key constraint. The syntax of the look-up constraint is $\eclookup{G'_2}{G'_1}{H'_1}{H'_2}$, where the values in the vector $G'_2$ are determined by looking up the corresponding value of vector $G'_1$ in vector $H'_1$ and returning the corresponding value in vector $H'_2$. In Figure~\ref{fig:main_example} we have $\eclookup{\range{T_{4}}{\rangeall}{3}}{\range{T_{4}}{\rangeall}{2}}{\range{T_{1}}{\rangeall}{1}}{\range{T_{1}}{\rangeall}{2}}$. This implies a foreign-key constraint $\ecfkey{G'_1}{H'_1}$ where each of the values in vector $G'_1$ must appear in $H'_1$ for the look-up to work. 

Another form of redundancy is because of \textit{equivalent} constraints.
Two constraints are equivalent if they imply each other. More formally, $\forall G'_1, ..., G'_n$ s.t. $\{G''_1, ..., G''_n\} = \{G'_1, ..., G'_n\},\, c_1(G'_1, ... G'_n)$ $\leftrightarrow c_2(G''_1, ..., G''_n)$.
...
A good way to deal with equivalent constraints is to identify in a generic way a \textit{canonical} constraint among the equivalent ones. For example, for equivalent constraints of the same \template, the constraint where the groups are ordered by table, orientation and row/column identifier. For equivalent constraints of different templates one could define heuristics such as that some templates are preferred over others, e.g. binary sum over n-ary sum, sum over difference, etc.
...
Looking at the template signatures in Table~\ref{table:constraints} one can see that some signatures depend on other constraints. This dependency also translates into the template implying the other templace. In Inductive Logic Programming, one often exploits implications between constraints to structure the search space \cite{luc_book}. Similarly, we can use these implications to define an ordering over the templates, such that the more general constraints (the requirements) are verified before the more specific constraints. Figure~\ref{fig:learning_order} shows the dependency graph extracted from the signature definitions in Table~\ref{table:constraints}. 
Constraint templates that have no dependencies are omitted. Because there are no equivalences or loops in the signatures, the resulting graph is a directed acyclic graph (DAG).
