...
\sergey{Needs to be checked, bullet points for Luc and Tias to rework introduction}\\
...
\begin{itemize}
  ...
  \samuel{Vector also has a length; Group has a size}
  \item
...
\samuel{clarify, e.g. what is group variable}
...
\tias{Reformulate: describe there's an naive approach...}
...
\tias{Split extensions and group detection, clarify group detection, example (e.g. currency for extensions)}
...
\\\\
\sergey{Can be parallelized by looking at connected components and respecting order}
...
\tias{...such that at least one subgroup satisfies the signature}
...
\tias{Add paragraph on enforcement of group generation}
...
\tias{Mention trade-off in the approach}
