...
\begin{definition} \textit{Tabular Constraint Learning.}\label{def:problem_statement}
  Given ${\cal G}$ a set of groups derived from fully instantiated tables ${\cal T}$ (all cells have values) and a set of {\template}s ${\cal C}$ ; find the set of all constraints $s(G_1', ..., G_n')$ that match the syntax of a \template in ${\cal C}$ and where the groups $G_1', ... , G_n'$ with $\forall i, G_i' \subseteq G_i, G_i \in {\cal G}$ satisfy both the signature and the definition of that \template.
\end{definition}
% {\bf Given }
% \begin{enumerate}
% \item
