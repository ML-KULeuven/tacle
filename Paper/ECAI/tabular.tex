...
\begin{figure*}[thb]
  ...
  \caption{
    ...
    Most tables only contain type-consistent columns, however, the data in table~$T_2$ can be used as both column- and row vectors.
    This example was created by combining several smaller Excel sheets originating from an exercise session.
    \samuel{A: I guess you want to save space by having everything in one figure, but it might be useful to have a smaller figure on the first page just to explain this example in a bit more detail (or use a smaller example here); it's quite overwhelming now}
  }
...
\caption{
  ...
  $\discrete(x)$ is a shortcut for: $\textual(x) \lor \numeric(x)$.  {\small\luc{there are some fancy constraints here. it might be good to say something about them ... like an example of one of the more complex ones in the text ... maybe state what conditional means, what aggregate ... }}
}
\label{table:constraints}
  ...
  \begin{tabularx}{\textwidth}{l X X}
    ...
    $\ecfkey{\sg_{fk}}{\sg_{pk}}^*$
      ...
      & Every value in~$\sg_{fk}$ also exist in~$\sg_{pk}$ \\ \hline
...
\\\\
\samuel{A: I'm a bit confused by the fact that you seem to be mixing the formal way of writing these triples with textual explanation, both in the example in the text and in the table; I'd prefer to see the formal thing and then get the explanation, as that's what I'd need to use your approach}
...
Definitions of tables were added manually, providing a way to compare results and overcoming shortcomings in the table detection algorithm (in some harder cases with $\mathit{None}$ values, blocks where also provided manually). \samuel{chec}
...
\subsubsection*{Q1. How accurate \sname?}
