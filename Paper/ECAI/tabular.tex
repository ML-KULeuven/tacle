...
Let us now define what is a tabular solution for a \template in general. Let $C$ denote a \template, $S$ a mapping from variables to the subgroups of \groups, and $C^S$ the constraint $C$, in which all variables are substituted with the corresponding subgroups from $S$, then $S$ is a \textit{tabular solution} to $C$ iff both $\CSignature^S$ and $\CFunction^S$ hold.
...
Finding candidate groups for a \template~$c$ can be seen as a Constraint Satisfaction Problem (CSP) dependent on the \CSignature of $c$.
...
Every \template~$C$ needs to have an implementation that finds subgroup assignments, given set of tuples each containing a candidate group for every variable in~$C$.
