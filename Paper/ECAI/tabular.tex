...
\caption{
  ...
  \begin{tabularx}{\textwidth}{l X X}
    ...
    $\ecfkey{\sg_{fk}}{\sg_{pk}}^*$
      & $\sg_{fk}$ and~$\sg_{pk}$ are both $\discrete$; $\ptype(\sg_{fk}) = \ptype(\sg_{pk})$; $\ptable(\sg_{fk}) \neq \ptable(\sg_{pk})$; and $\ecalldiff{\sg_{pk}}$
