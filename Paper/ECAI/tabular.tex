...
\institute{KU Leuven\\ {*} Shared first author}
...
%tias{I am not defining a new problem for 'most specific canonical tabular constraint learning' because we loosely showed how implications and equivalences would be expressed, but not how to express canonicity and we did not claim that most specific is always best.}
%tias{Note also that our method finds ALL constraints; it uses specificity to faster traverse the search space, but the resulting set of constraints found would be the same, whether or not using the specificity (see 4.2.1).}
...
%tias{Moved the discussion from 4.4.3 to here, where it better fits. Plus, I find the O(1) claim too strong! O() is worst-case time complexity, not average case}
%the time to check if a subassignment satisfies a constraint definition with a fail-fast implementation requires constant time\footnote{If the number of entries to look at is bounded by a constant, the vector lengths are smaller than the number of subassignments to test ($a$) and the time to test subassignments that satisfy the constraint is $O(a)$, then the time to test a subassignment is amortized $O(1)$} in most cases and does not depend on the length of the vectors.
...
% \end{verbatim}
 ...
 %$G_1 = +(G_2, G_3) \leftrightarrow G_1 = +(G_3, G_2)$. samuel{tias{How are they stopped in the code? should be part of the pseud-code because not part of $Test(X)$?} will make it part of pseudo-code}
...
% $If \lnot G_{O_1} < G_{O_2}: Reject~\#~Canonicity$tias{ill-defined, $G_{O_1}$ is a group and $<$ not defined on it (on vector identifiers rather than content?)}\\
% $If data(G_R) = data(G_{O_1}) \bullet data(G_{O_2}): Accept$ tias{does not show that reasons over content of vectors}
% \\\\
% Method $FindSubgroups(Assignments)$\\
% $Return \{ (G_R', G_{O_1}', G_{O_2}') | \exists (G_R, G_{O_1}, G_{O_2}) \in Assignments \land G_R' \subseteq G_R \land G_{O_1}' \subseteq G_{O_1} \land G_{O_2}' \subseteq G_{O_2} \land Test(G_R', G_{O_1}', G_{O_2}') \}$ tias{does not enforce size of subgroups (e.g. vectors)?}
...
Moreover, among the semantically equivalent constraints product (\ecprod{\sbl{R}'}{\sbl{1}'}{\sbl{2}'}) and division (\eccalc{\sbl{1}'}{\sbl{R}' / \sbl{2}'}) we only added product, as well as only difference (\ecdiff{\sbl{r}}{\sbl{1}}{\sbl{2}}) and not sum (\eccalc{\sbl{1}}{\sbl{r} + \sbl{2}}). However, division and sum could easily be added, and they could equivalently be added in post-processing based on the matching product and difference constraints.
...
For \textit{LOOKUP}, aggregates and \textit{PROJECT} we use optimized implementations to find subassignments instead of generic generate-and-test.

Given a superblock assignment, the implementation for \eclookup{\sbl{r}}{\sbl{fk}}{\sbl{pk}}{\sbl{val}} first generates candidate vectors for \sbl{pk} and \sbl{val}, then populates a hash table that allows it to quickly look up keys, afterwards it generates candidate vectors for the remaining arguments and, finally, checks which subassignments are valid.
...
%tias{Just below this comment I commented out the paragraph that FK,Lookup,proj have optimized methods becuase it does not properly explain what is used instead. That creates confusion and makes our method not reproducible. Leave out until it is rewritten with sufficient information.}
%As an optimization, for \textit{FOREIGNKEY}, \textit{LOOKUP}, aggregates and \textit{PROJECT} we use custom methods to find subassignments.
%Instead of the more generic generate-and-test implementation, we use, for example, caching, maximal range-finding and partial tests. tias{Really, this is unacceptably vague. I have no clue what you mean with that, meaning that our research is not reproducable and hence not scientific}
%While conditional aggregates use generate-and-test, they employ similar optimizations in their test method.


%\paragraph{Theoretical analysis of runtime, this does not belong in experiments}tias{move to technical section}sam{Has to be moved yet to 'Constraints' section, explaining fail-fast}
...
% Extending the system with a new constraint template amounts to tias{TODO: samuel, explain how to define syntax; signature-for-groupgen; signature-for-def; definition}
...
Blocks are then detected automatically using the block detection algorithm (Section~\ref{sec:make_groups}).
...
%and any intended constraint involving one such vector was also added for the equivalent vectors. \{They are usually not copied} \{'usually' is not acceptable in a scientific paper. Why would you sometimes do it and sometimes not? Preferably always or never copy, and if not specify when and why exactly you deviate.}
...
% tias{Better move to 4.3.2? or have there too anyway, no new facts in 'experiment' section}samuel{?}
...
% tias{unclear, this is about phase 2, the number to test, or the actual testing of each? explain and relate to Sec 4.3.2 what aspect you are discussing here} to be tested depends on the number of vectors in the spreadsheets and the arity of the constraint.
...
\noindent 
These experiments with generated data have given us a better understanding of what effect the properties of vectors and blocks can have on runtime and hence on the ability of the algorithm to finish within reasonable time. We next investigate the behaviour of our method on real spreadsheets.
...
% Notice that for the data category the average number of cells is higher while the number of intended constraints and tables is lower compared to the other categories. tias{Weird here because purpose unclear, point to this when discussing some result for which this is important}
...
\textbf{Recall} & 0.88 & 0.85 & 0.88 & 0.87 & 1.00 & 1.00 \\ \hline
...
\textbf{Runtime (s)} & 0.91 & 0.10 & 0.87 & 0.04 & 0.79 & 0.20
\end{tabular}
\end{table}}
...
Therefore, \sname is able to find all constraints for supported constraint templates, which shows that our method is able to learn all constraints using the bias chosen (the formalism of signature and definition). %tias{'confirms'? We never write anywhere that we doubt this (and if we did I probably removed it : ///) Alternative could be something like "shows that our method is able to learn all constraints using the bias chosen (the formalism of signature and definition)"?}
...
The intended constraints that are not discovered correspond with nested constraints (e.g. $R = \frac{W}{(L/100)^2}$), variants of supported constraints (e.g. series that do not start at 1) or more complex versions of supported constraints (e.g. conditional aggregates whose conditions are constant values). %tias{if you can clarify the latter, please do; if too difficult OK but 'automatically infer' sounds.. unusual for Excel}
...
In this section, we illustrate how various applications can build on our method for solving the tabular constraint learning problem.
...
\added{A final direction is that the current approach assumes noise-free data (but see the part on error correction); in inductive logic programming there has been much work on how to handle noisy data such as allowing a tolerance level $\epsilon$ when a rule or a constraint $\sum_{i=1}^n b_i = c$ matches up to $\epsilon$: $|\sum_{i=1}^n b_i - c| \leq \epsilon$, while in constraint satisfaction a popular alternative is to consider \textit{soft} constraints that can be violated at a cost, which is a real value and then the task is to find top-$k$ best matching constraints. }
