...
Let us describe the key steps and functions in Algorithm \ref{algo:tcl}. Essentially, the algorithm has three steps: constraint ordering, candidate group generation and subgroup satisfaction search, which is in line with the ``generate-and-test'' paradigm that is well-known in AI \cite{whaisasp}. Let us elaborate on each step in detail.

\paragraph{Constraint ordering} $\constrainttorder(\constraints,\dependencies)$ \sergey{explaination that solutions to certain constraints allows us to prune solutions to other constraints -- subsumption, condensed representation}
...
A number of redundant constraints ($33.33\%$ of the total) are also found, which, as we will show, is rather high compared to other spreadsheets.
