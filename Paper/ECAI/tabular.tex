...
\tias{Make general intro of subsections}
...
\tias{Mention spreadsheet-specific hacks in the exps, not here}
...
\tias{Samuel, what does this mean?}
...
\tias{1) all templates, order so only once; 2) generate groups that satisfy signature; 3) generate subgroups that satisfy definition}
...
\begin{algorithm}[thb]
  ...
  \State $v_1, ..., v_n =$ variables of $T$ \tias{variables not defined, only nr of groups important?}
...
We now describe each of the three steps in more detail.
...
\\\\
Constraint templates have to be treated respecting the order, i.e. learning the most general ones first.
...
\tias{find groups, each $n$ groups and $m$ arguments than m-choose-n candidates. instead of generate-and-test we will search for all valid groups. To do so, CSP with signature in logical constraints and properties of each group as data. Example should use constraint syntax}
...
If a numeric group is assigned to $v_r$ then some subgroups may have type float while other may be of type integer, therefore, this this group cannot be discarded.

For some constraints (e.g. sum, max, count) that share the same \CSignature the same set of assignments~$A$ will be found. \tias{so?}
...
\subsubsection{Satisfying subgroups}
\tias{not set of assignment but single assignment $a$?}
\tias{requires checking for which of the subgroups of the groups the constraint is satisfied. many subgroups of the groups will not satisfy signature, hence no generate-and-test. Instead, we use again satisfiability to both search for the subgroups and verify satisfiab of the sign and definition.} 
...
\section{Examples}
\subsection{Group generation CSP}
variables from syntax, domain from input groups, filtered using type
...
constraints: $\plength(O_1) = \plength(O_2) \geq 2, \pcols(R) = 1, \prows(R) = 1$ % excluded not partial because we don't focus on it much
