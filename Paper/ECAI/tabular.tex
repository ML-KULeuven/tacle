...
% why difficult
In this paper,  we investigate whether knowledge discovery techniques can be used to infer constraints (formulas and other relations) from raw spreadsheet data.
From a machine learning point of view this is an unconventional problem because 
...
\begin{itemize}
\item we define the tabular constraint learning problem, where the goal is to learn constraints (functions and relations) that range over entire rows or columns. This is an unsupervised learning problem;
\item we propose a 2-stage generate and test method that reasons for each candidate constraint over the maximally contiguous submatrices in the first stage, and only in the second stage looks at individual rows and columns and their content;
\item furthermore, in the first stage we use a constraint solver to efficiently enumerate all tuples of submatrices compatible with the candidate constraint its arguments, using backtracking search and forward checking;
\item experiments on a range of publicly available spreadsheets show that the system is able to extract constraints with high precision and recall in an unsupervised manner. 
\end{itemize}
