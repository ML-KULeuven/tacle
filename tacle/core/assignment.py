...
class Variable:
    def __init__(self, name, prime=False, vector=False, types={GType.int, GType.float, GType.string}):
        self.name = name
        self.prime= prime
        self.vector = vector
        if len(types) == 0:
...
class Source:
    ...
    def candidates(self, groups, target_group, solutions, filters):
        return self._complete([{}], groups, target_group, filters, solutions)

    def _complete(self, assignments, groups, target_group, filters, solutions):
        result = []

        if bool(target_group):
            assignments = [target_group]#problem might arise when dependency is intruduced
            #assignments.append(target_group)
...
class ConstraintSource(Source):
    ...
    def candidates(self, groups, solutions, filters):
        ...
        print("Initial assignment: {}".format(assignments))
        #list of dictionary key=variable.name value= assignment
        #>[{'PK': T2[:, 1]}, {'PK': T1[:, 4]}, {'PK': T1[:, 5]}, {'PK': T1[:, 6]}]
        print("After _complete : {}".format(self._complete(assignments, groups, filters, solutions)))
