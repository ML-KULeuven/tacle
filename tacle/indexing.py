...
class Typing(object):
    ...
    def lowest_common_ancestor(cell_type1, cell_type2):
        if cell_type1 == cell_type2:
            return cell_type1
        if cell_type1 is None or cell_type2 is None: #when both og them are blank e.g. ""
            return None
    ...
    @staticmethod
    #returning the super_type of the cell_type
    def max(cell_types):
...
class Range(object):
    ...
    def minus(self, other):
        ...
        x1= self.x1 if self.x1==other.x1 else self.x0+abs(other.x1- self.x1)
        y1= self.y1 if self.y1==other.y1 else self.y0+abs(other.y1- self.y1)

        return self.from_coordinates(x0, y0, x1, y1)
...
class Table(object):
    def __init__(self, data, type_data, t_range, header, name=None, orientations=None):
        ...
        self.header = header if (any(orientation in [Orientation.vertical] for orientation in orientations) and header is not None and header.height==1) \
                            or (any(orientation in [Orientation.horizontal] for orientation in orientations) and header is not None and header.width ==1) \
                            else None
