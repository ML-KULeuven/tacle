...
def get_groups(data, indexing_data):
    ...
    for table_description in indexing_data["Tables"]:
        bounds = Bounds(table_description["Bounds"])
        data_subset = bounds.subset(data)
        header_bounds = Bounds(table_description["Header"]) if table_description["Header"] is not None else None
        header_data = header_bounds.subset(data) if table_description["Header"] is not None else None
        name = table_description["Name"]
        orientation = None
        if "Orientation" in table_description:
            o_string = table_description["Orientation"].lower()
            if o_string == "row" or o_string == "horizontal":
                ...
                orientation = Orientation.VERTICAL
        table_dict[name] = Table(name, data_subset, header_data, orientation)
...
def create_index_group(data, table, index, orientation):

    #creating group.table
    bounds = Bounds(table.range.as_legacy_bounds().bounds)
    data_subset = bounds.subset(data)#entire data, not partial
    header_bounds = Bounds(table.header.as_legacy_bounds().bounds) if \
                        table.header.as_legacy_bounds().bounds is not None else None
    header_data = header_bounds.subset(data) if table.header.as_legacy_bounds().bounds is not None else None
    name = table.name

    group_table= Table(name, data_subset, header_data, orientation)


    #creating group
    bound= table.range.vector_range(index, orientation).as_legacy_list(orientation)

    from tacle.indexing import Typing
    g_types = [GType.int if Typing.root(gt) == "numeric" else GType.string
               for gt in table.get_vector_type(index, orientation)]

    group= create_group(bound, group_table, g_types)
    return group
