...
class MinizincAssignmentStrategy(DictAssignmentStrategy):
    ...
    def _get_assignments(groups, constraint, filename):
        ...
        data = generator.generate_group_properties(groups) + generator.generate_constraints(constraint, filename)
...
class MinizincSolvingStrategy(DictSolvingStrategy):
    def __init__(self):
        ...
        def sum_columns(constraint, assignments, solutions):
            ...
            assignment_tuples = [(a, local(filename.format("row" if a["X"].row else "column"))) for a in assignments]
