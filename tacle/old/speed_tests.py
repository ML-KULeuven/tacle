...
class SpeedTestId:
    ...
    def parse(input_string):
        ...
        return SpeedTestId(
            list(TableSpec.parse(table_part) for table_part in table_parts)
        )
...
def generate_random(test_id: SpeedTestId):
        ...
        for size, b_type in table.block_types:
            for _ in range(size):
                ...
                if b_type == GType.int:
                    ...
                    word = lambda: "".join(
                        letter() for _ in range(np.random.randint(5, 15))
                    )
    ...
    for i in range(max_rows):
        row = list(
            str(column_data[col][i]) if len(column_data[col]) > i else ""
...
def generate_blocks(test_id: SpeedTestId):
    ...
    for i, table in enumerate(test_id.tables):
        ...
        index += table.cols
        tables.append(
            {"Name": "T{}".format(i + 1), "Bounds": bounds, "Orientation": "Column"}
        )
    ...
    for i, table in enumerate(test_id.tables):
        ...
        for size, _ in table.block_types:
            blocks.append(
                {"Table": "T{}".format(i + 1), "Bounds": [":", index, index + size - 1]}
            )
            index += size
...
def generate_experiments():
    ...
    for cols in increment(min_power, number_power):
        rows = default
        tables = [
            TableSpec(rows).add_block(cols, gtype),
            TableSpec(rows).add_block(min_table, gtype),
        ]
    ...
    for block_size in reversed(increment(0, block_power)):
        if block_size <= vectors:
            ...
            cat_3.append(
                SpeedTestId([table, TableSpec(rows).add_block(min_table, gtype)])
            )
...
def draw_figures(tables):
    ...
    if "cat1" in tables:
        table = tables["cat1"]
        scatter_1 = ScatterData(
            "(a) Number of vectors", list(test_id.tables[0].cols for test_id in table)
        )
        scatter_1.add_data(
            "All constraints",
            list(np.average(series["total"]) for series in table.values()),
        )
    ...
    if "cat2" in tables:
        ...
        scatter_2 = ScatterData(
            "(b) Vector size", list(test_id.tables[0].rows for test_id in table)
        )
        ...
        no_rank_run_times = list(
            total - rank for total, rank in zip(total_run_times, rank_run_times)
        )
    ...
    if "cat3" in tables:
        ...
        aggregates = list(
            sum(np.average(series[c]) for c in Aggregate.instances())
            for series in table.values()
        )
        non_aggregates = list(
            total - aggregate for total, aggregate in zip(total_times, aggregates)
        )
...
def get_tables(categories, import_file=None, runs=1):
    ...
    if import_file is None:
        ...
        log_file_path = os.path.join(
            path, "{}.log".format(time.strftime("%Y%m%d_%H%M%S"))
        )
            ...
            for run in range(runs):
                ...
                for test_id, (csv_path, blocks_path) in paths.items():
                    ...
                    print(
                        ...
                        "Name",
                        "Total time",
                        *[str(c) for c in task.constraints],
                        file=log_file,
                        sep="\t"
                    )
                    print(
...
def main():
    ...
    tables = get_tables(
        generate_experiments(), import_file=import_file
    )  # import_file="20160914_141603.log")
    print_tables(tables)
    draw_figures(tables)
